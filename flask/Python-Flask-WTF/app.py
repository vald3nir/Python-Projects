...
class CreateUserForm(FlaskForm):
    ...
    confirm_password = PasswordField(label='Confirm Password', validators=[
        DataRequired(message='*Required'), EqualTo('password', message='Both password fields must be equal!')
    ])
...
class GreetUserForm(FlaskForm):
    username = StringField(label='Enter Your Name:', validators=[
        DataRequired(), Length(min=5, max=64, message='Name length must be between %(min)d and %(max)dcharacters')
    ])
    submit = SubmitField(label=('Submit'))
